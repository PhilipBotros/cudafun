...
__constant__ float d_F[KERNEL_SIZE][KERNEL_SIZE];


// Kernel code
// A is input matrix, K is conv kernel, C is output matrix 
// Assume A=MxN and B=KxK -> C=MxN
__global__ void basicConv(float *d_A, float *d_C, int M, int N, int K) {
...
int main() {
    ...
    basicConv<<<gridSize, blockSize>>>(d_A, d_C, M, N, K);
