...
__global__ void softmax1D(float *in, float *out, int dim) {
    ...
    // Compute exp(in[idx]) for valid indices, else set to zero.
    float e_x = 0.0f;
    if (idx < dim) {
        e_x = exp(in[idx]);
        smem[tx] = e_x;
    } else {
        smem[tx] = 0.0f; // All threads write to shared memory.
    }
    __syncthreads();
    
    // Block-wise reduction: sum the exponentials within the block.
    ...
    // Ensure all block sums are written.
    grid.sync();

    // Global thread 0 computes the total sum from block sums.
    if (grid.thread_rank() == 0) {
        float total_sum = 0.0f;
        for (int i = 0; i < gridDim.x; ++i) {
            total_sum += out[i];
        }
        // Store the total sum in out[0] for later use.
        out[0] = total_sum;
    }
    
    // Sync again to ensure the total sum is available to all threads.
    grid.sync();
    
    // Finally, compute the softmax only for valid indices.
    if (idx < dim) {
        out[idx] = e_x / out[0];
    }
}
...
int main() {
    ...
    // Launch the kernel cooperatively with shared memory size equal to blockDim * sizeof(float)
    ...
    // Output the input and the softmax result.
    std::cout << "Input:\n";
    for (int i = 0; i < dim; ++i) {
        std::cout << h_in[i] << " ";
    }
...
__global__ void softmax1D(float *in, float *out, int dim) {
    ...
    __shared__ float blockSums[32];  // Enough to hold one partial sum per warp.
